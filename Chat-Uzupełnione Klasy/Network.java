...
public class Network {
    ...
    public void run() {
        while (true) {
            if (!receive()) {
                // Jeśli metoda receive zwróciła false, wówczas trzeba zamknąć socket
    ...
    public boolean startClient(String ip) {
        ...
        packet.type = PacketType.REGISTER;
        return send(packet);
    }
    ...
    public boolean receive() {
        ...
        // W zależności od typu pakietu wykonaj akcje
        if (packet.type == PacketType.REGISTER) {
            // Analogicznie akcje jak w  metodzie startClient
            friendsAddress = dp.getAddress();
            friendsPort = dp.getPort();
        } else if (packet.type == PacketType.MESSAGE) {
            // dodanie pakietu do klasy comversation
